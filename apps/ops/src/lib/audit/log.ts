...
// ── Filesystem (local dev) — append-only NDJSON ───────────────────────────────
// One JSON line per entry. Never rewrites the file, so no data is ever lost.
...
// One blob per entry keyed by its UUID — no read-modify-write, no race condition.
// Listing blobs for reads is O(n) but fine at ops-team traffic levels.
...
async function blobsRead(limit: number): Promise<AuditEntry[]> {
  ...
  // Blobs list is unordered — fetch all, sort by timestamp, return newest first.
...
export async function appendAuditEntry(
  ...
  try {
    ...
    // Audit failure must never crash the calling request. Log and continue.
