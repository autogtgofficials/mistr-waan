...
export function MechanicRow({
  ...
  mechanic: Mechanic;
  onOpenDetail: (m: Mechanic) => void;
  onPatched: (m: Mechanic) => void;
}) {
  const [pending, startTransition] = useTransition();
  const [notesOpen, setNotesOpen] = useState(false);
  const [notesDraft, setNotesDraft] = useState(mechanic.notes ?? "");

  async function patch(p: { onboardingStatus?: OnboardingStatus; notes?: string | null }) {
    const res = await fetch(`/api/mechanics/${encodeURIComponent(mechanic.id)}`, {
      method: "PATCH",
  ...
  const coarseServices = m.services.filter((s) => s !== "unknown");
  const detailedCount = m.detailedServices?.length ?? 0;
  const callCount = m.callLog?.length ?? 0;
  const coverageCount = m.coverageAreas?.length ?? 0;
  const followUpAt = m.nextFollowUpAt ? new Date(m.nextFollowUpAt) : null;
  const followUpDays = followUpAt
    ? Math.round((stripTime(followUpAt).getTime() - stripTime(new Date()).getTime()) / 86400_000)
    : null;
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1">
        ...
        {coverageCount > 0 ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
            ...
            {coverageCount} area{coverageCount === 1 ? "" : "s"}
          </span>
        ) : null}
      </div>
      ...
      <footer className="mt-3 flex flex-wrap items-center gap-2 border-t border-border-subtle pt-3">
        ...
        <button
          ...
          className="ms-auto inline-flex items-center gap-1 rounded-md bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground hover:bg-pulse-600"
        >
